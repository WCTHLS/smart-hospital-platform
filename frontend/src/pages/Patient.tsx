...
import { CheckCircle2, MapPin, Clock, Bell, FileText, LogOut, Stethoscope, Clipboard, Receipt, Activity } from "lucide-react";
import { api } from "../lib/api";
import { useJourney } from "../lib/store";
import { clearPortalPatient, getPortalPatient } from "../lib/patientAuth";
      ...
      <div className="space-y-4">
        ...
        <Card className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h4 className="font-bold text-xs uppercase tracking-wider" style={{ color: "var(--dim)" }}>Today's visits</h4>
            <button className="btn ghost sm shrink-0" onClick={() => nav("/patient/appointments/book?redirect=/patient")}>Book appointment</button>
          </div>
          {!todayAppointments.length && !todayEncounters.length && <div className="holo text-xs">No visits scheduled for today.</div>}
          <div className="max-h-[320px] space-y-2 overflow-y-auto pr-1">
            {todayAppointments.map((appointment: any) => <div className="rounded-xl border p-2.5 text-xs" style={{ borderColor: "var(--glass-border)" }} key={appointment.appointment_id}>
              <div className="flex items-center justify-between gap-3">
              <div className="min-w-0 flex-1">
              <div className="truncate font-bold text-white">{appointment.doctor?.name ?? "Assigned doctor"}</div>
              <div className="mt-1" style={{ color: "var(--muted)" }}>{appointment.specialty} · {appointment.scheduled_start.slice(11, 16)}</div>
              <div className="mt-1" style={{ color: "var(--muted)" }}>Reason: {appointment.reason || "Not provided"}</div>
              </div>
              <button className="btn g shrink-0 text-xs" onClick={() => nav(`/patient/checkin?appointment=${appointment.appointment_id}`)}>Check in</button>
              </div>
            </div>)}
            {todayEncounters.map((e: any) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1" style={{ color: "var(--muted)" }}>Reason: {e.reason || "Not provided"}</div>
                </button>
              );
            })}
          </div>
        ...
        <Card className="space-y-3">
          <h4 className="font-bold text-xs uppercase tracking-wider" style={{ color: "var(--dim)" }}>Past visits</h4>
          {!pastEncounters.length && <div className="holo text-xs">No past visits available.</div>}
          <div className="max-h-[320px] space-y-2 overflow-y-auto pr-1">
            {pastEncounters.map((e: any) => {
              ...
              return <button
                ...
                style={{ borderColor: isActive ? "var(--line2)" : "var(--glass-border)", background: isActive ? "rgba(52,225,232,0.05)" : "rgba(255,255,255,0.01)" }}
              >
                <div className="mb-1 flex items-center justify-between"><span className="font-bold text-white">{e.date}</span><Tag tone="green">{e.status}</Tag></div>
                <div style={{ color: "var(--muted)" }}>{e.department} department</div>
                <div className="mt-1" style={{ color: "var(--muted)" }}>Reason: {e.reason || "Not provided"}</div>
              </button>;
            })}
